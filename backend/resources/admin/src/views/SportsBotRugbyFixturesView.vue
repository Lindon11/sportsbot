<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between gap-3 flex-wrap">
      <div>
        <h1 class="text-2xl font-bold text-white">SportsBot Rugby Fixtures TV</h1>
        <p class="text-slate-400 text-sm mt-1">Preview cards and publish rugby TV listings to the RUGBY Telegram topic.</p>
      </div>
      <button
        @click="loadPreview"
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <div class="rounded-2xl bg-slate-800/50 border border-slate-700/50 p-4">
        <p class="text-slate-400 text-sm">Rugby Fixtures</p>
    ...
    <div class="rounded-2xl bg-slate-800/50 border border-slate-700/50 p-5 space-y-4">
      <div class="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h2 class="text-lg font-semibold text-white">Rugby Topic Preview</h2>
          <p class="text-xs text-slate-400">Route: RUGBY · one card per fixture · no inline buttons</p>
        </div>
        <div class="flex items-center gap-2">
          ...
          >
            {{ testingRoute ? 'Testing...' : 'Test Rugby Route' }}
          </button>
          <button
            @click="sendRugbyFixtures"
            :disabled="sending"
            class="px-4 py-2 rounded-xl bg-emerald-700 text-white hover:bg-emerald-600 disabled:opacity-60"
          >
            {{ sending ? 'Sending...' : 'Send to Rugby Topic' }}
          </button>
        </div>
      </div>

      <div v-if="routeStatus.fallback" class="rounded-xl border border-amber-600/40 bg-amber-500/10 p-3 text-sm text-amber-200">
        RUGBY is falling back to the default Telegram target. Assign the RUGBY route in Telegram Routes before sending to the topic.
      </div>
    ...
    <div class="rounded-2xl bg-slate-800/50 border border-slate-700/50 p-5">
      <h2 class="text-lg font-semibold text-white mb-4">Recent RUGBY Sends</h2>
      <div v-if="recentMessages.length === 0" class="text-sm text-slate-400">No recent rugby fixture sends.</div>
...
async function loadRecentMessages() {
  try {
    const { data } = await api.get('/admin/sportsbot/telegram/messages', {
      params: { route_key: 'RUGBY', limit: 20 },
    })
    recentMessages.value = data.messages || []
  } catch (error) {
    toast.error(error?.response?.data?.message || 'Failed to load recent rugby sends')
  }
}
...
async function loadPreview() {
  ...
  try {
    ...
    toast.error(error?.response?.data?.message || 'Failed to load rugby fixture preview')
  } finally {
...
async function testRoute() {
  ...
  try {
    ...
    toast.success(`Rugby route test sent (${(data.results || []).length} target(s))`)
    await loadRecentMessages()
  } catch (error) {
    toast.error(error?.response?.data?.error || 'Rugby route test failed')
  } finally {
    testingRoute.value = false
  }
}

async function sendRugbyFixtures() {
  sending.value = true
  try {
    const { data } = await api.post('/admin/sportsbot/rugby-fixtures/send', {
    ...
    toast.success(`Rugby fixtures sent (${(data.results || []).length} photo post(s))`)
    await loadRecentMessages()
  } catch (error) {
    toast.error(error?.response?.data?.error || 'Failed to send rugby fixtures')
  } finally {
    sending.value = false
  }
}
