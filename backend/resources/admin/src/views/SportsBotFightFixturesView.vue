  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between gap-3 flex-wrap">
      <div>
        ...
        <p class="text-slate-400 text-sm mt-1">Preview upcoming boxing, UFC, BKFC and PPV listings for the FIGHTS Telegram topic.</p>
      </div>
      <button
        @click="loadPreview"
    ...
    <div class="rounded-2xl bg-slate-800/50 border border-slate-700/50 p-5 space-y-4">
      <div class="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p class="text-xs text-slate-400">Route: FIGHTS · one card per event · no inline buttons</p>
        </div>
        <div class="flex items-center gap-2">
          ...
          <button @click="testRoute" :disabled="testingRoute" class="px-4 py-2 rounded-xl bg-cyan-700 text-white hover:bg-cyan-600 disabled:opacity-60">
            {{ testingRoute ? 'Testing...' : 'Test Fights Route' }}
          </button>
          <button @click="sendFightFixtures" :disabled="sending" class="px-4 py-2 rounded-xl bg-emerald-700 text-white hover:bg-emerald-600 disabled:opacity-60">
            {{ sending ? 'Sending...' : 'Send to Fights Topic' }}
          </button>
        </div>
      </div>

      <div v-if="routeStatus.fallback" class="rounded-xl border border-amber-600/40 bg-amber-500/10 p-3 text-sm text-amber-200">
        FIGHTS is falling back to the default Telegram target. Assign the FIGHTS route in Telegram Routes before sending to the topic.
      </div>
    ...
    <div class="rounded-2xl bg-slate-800/50 border border-slate-700/50 p-5">
      <h2 class="text-lg font-semibold text-white mb-4">Recent FIGHTS Sends</h2>
...
async function loadRecentMessages() {
  try {
    const { data } = await api.get('/admin/sportsbot/telegram/messages', {
      params: { route_key: 'FIGHTS', limit: 20 },
    })
...
async function testRoute() {
  ...
  try {
    ...
    toast.success(`Fights route test sent (${(data.results || []).length} target(s))`)
    await loadRecentMessages()
  } catch (error) {
