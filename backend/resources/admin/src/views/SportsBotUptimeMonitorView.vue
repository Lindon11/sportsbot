        ...
        <div class="flex items-center gap-3 min-w-0 flex-1">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              ...
              <span :class="site.status === 'online' ? 'text-emerald-400' : site.status === 'offline' ? 'text-red-400' : 'text-slate-400'" class="text-sm font-medium">{{ site.status }}</span>
            </div>
      ...
      <div class="bg-slate-800 rounded-2xl p-6 w-full max-w-md border border-slate-700">
        ...
        <div class="space-y-3">
          ...
          <div class="grid grid-cols-3 gap-3">
            <label class="block">
              <span class="text-slate-400 text-xs block mb-1">Interval (s)</span>
            ...
            <label class="block">
              <span class="text-slate-400 text-xs block mb-1">Timeout (s)</span>
              <input v-model.number="form.timeout_seconds" type="number" class="w-full rounded-xl bg-slate-950 border border-slate-700 text-white px-3 py-2" min="3">
            </label>
            <label class="block">
              <span class="text-slate-400 text-xs block mb-1">Fail threshold</span>
