...
{
    $timestamp = trim((string) ($event['strTimestamp'] ?? $event['strEventTimestamp'] ?? $event['strTimeStamp'] ?? ''));
...
function fb_card_caption_time_label(array $item): string
{
    ...
    if ($startsAt !== '') {
        try {
            ...
            // Fall through to raw field parsing below.
        }
    }
...
function fb_sport_display_label(string $sport): string
{
...
function fb_card_tv_event_caption(array $page): string
{
    ...
    $sport = fb_sport_display_label((string) ($event['sport'] ?? $page['sport'] ?? 'Sport'));
